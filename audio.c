...
int16_t pdmInputBuffer1[PDM_INPUT_BUFFER_LENGTH] = {0};
int16_t pdmInputBuffer2[PDM_INPUT_BUFFER_LENGTH] = {0};
int16_t pdmOutputBuffer[PDM_OUTPUT_BUFFER_LENGTH] = {0};
...
static void pdmEventHandler(nrfx_pdm_evt_t *event)
{
    ...
    static uint16_t outputBufferOffset = 0;

    if (event->buffer_requested) {
        pdmBufferSwitchFlag = !pdmBufferSwitchFlag;
        errorStatus = nrfx_pdm_buffer_set(pdmBufferSwitchFlag ? pdmInputBuffer1 : pdmInputBuffer2, PDM_INPUT_BUFFER_LENGTH);
        ASSERT(errorStatus == NRFX_SUCCESS);

        memcpy(pdmOutputBuffer + outputBufferOffset, event->buffer_released, sizeof(int16_t) * PDM_INPUT_BUFFER_LENGTH);
        outputBufferOffset += PDM_INPUT_BUFFER_LENGTH;

        if (outputBufferOffset == PDM_OUTPUT_BUFFER_LENGTH) {
            for (int i = 0; i < PDM_OUTPUT_BUFFER_LENGTH; i++) {
                fftInputBuffer[2*i] = pdmOutputBuffer[i] / 1.0f;
            }

            outputBufferOffset = 0;
            fftInputBufferReady = true;
        }
    }
    return;
}
...
void audioInit(void)
{
    ...
    nrfx_pdm_buffer_set(pdmInputBuffer1, PDM_INPUT_BUFFER_LENGTH);
    ASSERT(errorStatus == NRFX_SUCCESS);
